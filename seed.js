...
MongoClient.connect('mongodb://localhost/xboxone', (err, db) => {
  if (err) {
    console.error(err)
    process.exit(1)
  }
  const actionGames = db.collection('action-games')
  actionGames
    .insertMany([
      {
      ...
      },
      {
        ...
        'combat. Thhe game\'s combat emphasizes on \'flow\', a term referring to a ' +
    ...
    .then(() => console.log('Action games seeded!'))

  const fightingGames = db.collection('fighting-games')
  fightingGames
    .insertMany([
      {
      ...
      },
      {
        ...
        genre: 'Warner Bros. Interactive Entertainment',
        rating: 4,
        esrb: 'Mature',
    ...
    .then(() => console.log('Fighting games seeded!'))

  const shooterGames = db.collection('shooter-games')
  shooterGames
    .insertMany([
      {
        name: 'Battlefield 1',
    ...
    .then(() => console.log('Shooter games seeded!'))
    .then(() => db.close())
})
